...
- **1Password `tradeTracker.env` is a FIFO:** the app now merges `trade-tracker/.env`, `backend/.env`, and `tradeTracker.env` (including named pipes) into configuration before Pydantic runs. **If a variable is already set in your shell environment** (`export WEBULL_AUTO_SYNC_INTERVAL_HOURS=6`), that value wins and overrides file-based values—unset it or open a clean terminal if values look “stuck.” After changing 1Password fields, **restart the backend** so `get_settings` reloads.
